    ...
    if (_shockWave->isVisible()) {
        ...
        for (i = count-1; i >= 0; i--) {
            ...
            if (pow(diffx, 2) + pow(diffy, 2) <= pow(_shockWave->boundingBox().size.width * 0.5f, 2)) {
                ...
                //_explosion->retain();
                sprite->runAction((CCAction *) _explosion->clone()->autorelease()); //->autorelease()
                //_explosion->release();
...
void GameLayer::onTouchesBegan(const std::vector<Touch*> &ptouches, CCEvent* event) {
	...
	CCTouch *touch = (CCTouch *)ptouches.at(0);
    ...
    if (touch) {
        ...
        if (_bomb->isVisible()) {
            ...
            if (_bomb->getScale() > 0.3f) {
                ...
                CCLog("Before shockwave sequence");
                _shockWave->runAction((CCFiniteTimeAction*) _shockwaveSequence->clone()->autorelease()); //->autorelease()
                CCLog("After shockwave sequence");
                //_shockwaveSequence->release();
        ...
        //if no bomb currently on screen, create one
            ...
            _bomb->setOpacity(50);
           // _growBomb->retain();
            _bomb->runAction((CCAction *) _growBomb->clone()->autorelease()); //->autorelease()
            //_growBomb->release();
            CCSprite * child;
            child = (CCSprite *) _bomb->getChildByTag(kSpriteHalo);
            //_rotateSprite->retain();
            child->runAction((CCAction *) _rotateSprite->clone()->autorelease()); //->autorelease()
            //_rotateSprite->release();
            child = (CCSprite *) _bomb->getChildByTag(kSpriteSparkle);
            //_rotateSprite->retain();
            child->runAction((CCAction *) _rotateSprite->clone()->autorelease()); //->autorelease()
            //_rotateSprite->release();
...
void GameLayer::fallingObjectDone (CCNode* pSender) {
    ...
    if (pSender->getTag() == kSpriteMeteor) {
        ...
        //_groundHit->retain();
        pSender->runAction((CCAction*)_groundHit->clone()->autorelease()); //->autorelease()
        //_groundHit->release();
...
void GameLayer::resetHealth(void) {
    ...
    //_swingHealth->retain();
    health->runAction((CCAction *)_swingHealth->clone()->autorelease()); //->autorelease()
    //_swingHealth->release();
